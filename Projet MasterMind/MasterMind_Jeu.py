...
def _init_():
    ...
    def color(canvas):
        liste_canvas[ligne//nb_couleur][canvas].itemconfig(boule, fill=couleurs[liste_proposition[ligne//nb_couleur][canvas]%nb_couleur])
    ...
    def sauvegarder():
        ...
        code_secret2=[]
        tableau_label=[]
        liste_proposition2=liste_proposition.copy()
        code_secret2 = code_secret.copy()
        tableau_label2 = tableau_label
        fichier=open("sauvegarde0.txt","w")
        json.dump((liste_proposition2,code_secret2,tableau_label),fichier)
        fichier.close()
    ...
    liste_canvas = [[tk.Canvas(fenetre, width=45, height=45, bg="lightgrey") for i in range(taille_code)] for j in range(nb_ligne)]
    for j in range(nb_ligne):
        for i in range(taille_code):
            liste_canvas[j][i].grid(row=j+1, column=i+2)

    #ici on met dans chaque canvas créé précedement une boule blanche
    for j in range(nb_ligne):
        for i in range(taille_code):
            boule = liste_canvas[j][i].create_oval(10, 10, 40, 40, fill="white")
    ...
    Bouton_sauvegarder = tk.Button(fenetre, width=10, height=2, text = "Sauvegarder",bg="green",activebackground="green",command=sauvegarder)
...
#Les boutons "charger", ne sont pas finit (le boutton sauvegarder fonctionne, il creer un fichier mais ne sert a rien sans le boutton charger)


#    def charger():
#        global liste_proposition
#        global code_secret
#       global tableau_label
#        fichier = open("sauvegarde.txt","r")
#        liste_proposition = json.load(fichier)
#        code_secret = json.load(fichier)
#        tableau_label = json.load(fichier)
#         _init_()
