...
def _init2_():
    ...
    code_secret= [-1 for f in range(4)]
    code_secret1=[]

    def color_secret(canvas):
        tableau_secret[ligne//nb_couleur][canvas].itemconfig(boule, fill=couleurs[(code_secret[canvas])%nb_couleur])
    ...
    def validé():
        ...
        code_secret_bouton = tk.Button(fenetre,width=10, height=2, text= "Afficher", bg="lightpink", activebackground = coul_alé())
        code_secret_bouton.grid (row=13, column=6)

        cacher_code_secret1 = tk.Button(fenetre, width=10, height=2, text= "Cacher",  bg="lightyellow", activebackground = coul_alé())
        cacher_code_secret1.grid (row=14, column=6)
