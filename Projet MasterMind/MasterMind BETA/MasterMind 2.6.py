...
def _init_():
    ...
    # fct qui permet d'afficher le code secret
    ...
    #On créer une liste nul ([-1,-1,-1,-1]) pour chaque ligne 
    ...
    for i in range(0,nb_ligne+1):
        affiche_pions=tk.Label(fenetre,text="",bg="lightgrey")
        affiche_pions.grid(column=6,row=i+1)
        tableau_label.append(affiche_pions)
    ...
    code_secret_bouton = tk.Button(fenetre, height=2, text= "Afficher code-secret: ",command= afficher_code_secret)
    code_secret_bouton.grid (row=4, column=7)

    cacher_code_secret1 = tk.Button(fenetre, height=2, text= "Cacher code-secret: ", command= cache_code)
    cacher_code_secret1.grid (row=6, column=7)
    ...
    Bouton_charger = tk.Button(fenetre, width=10, height=2, text = "Charger",bg="lightgrey",activebackground="white", command= retour_arriere)
...
cadre1 = tk.Frame(fenetre_regle,bg='black', bd='3',relief="sunken") #Le cadre
...
deux_joueur.bind("<Button-1>",détruire)
deux_joueur.pack()
